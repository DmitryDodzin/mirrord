...
//             info!(
//                 "Agent exiting without having removed all the clients. Unpausing target
// container."             );
...
//     stream_sender: broadcast::Sender<BincodeMessage>,
...
//                 .map_err(|e| {
//                     warn!(
//                         "Failed to create TcpSnifferApi: {}, this could be due to kernel
// version.",                         e
//                     );
//                     e
//                 })
...
//         let _ = self
//             .stream_sender
//             .send(BincodeMessage::from_bincode(&response).unwrap());
...
//             while let Ok(message) = message_recev.recv() {
...
async fn main() -> Result<()> {
    ...
    let (a, _) = mpsc::channel(0);
    let (b, _) = mpsc::channel(0);
    let (c, _) = mpsc::channel(0);

    let _ = client::ClientConnection::create(1, None, false, a, b, c, Default::default()).await?;
