...
#[derive(Debug)]
pub enum MachineToken {
    /// V1 API key carries exactly the same data as [`Credentials`].
    V1(Credentials),
}

impl MachineToken {
    /// Retrieves the [`Credentials`] from this [`MachineToken`].
    ///
    /// Helper so you don't have to `match ci_api_key` everywhere to access it.
    pub fn credentials(&self) -> &Credentials {
        match self {
            MachineToken::V1(credentials) => credentials,
        }
    }
}

impl From<Credentials> for MachineToken {
    fn from(credentials: Credentials) -> Self {
        MachineToken::V1(credentials)
    }
}

impl bincode::Encode for MachineToken {
    fn encode<E: enc::Encoder>(&self, encoder: &mut E) -> Result<(), EncodeError> {
        use bincode::Encode;

        match self {
            MachineToken::V1(credentials) => Encode::encode(&credentials, encoder),
        }
    }
}
...
pub mod client {
    ...
    impl MachineToken {
        ...
        /// Helper to get the correct prefix for this [`MachineToken`].
        ///
        /// [`MachineToken`] gets encoded with this prefix.
        const fn prefix(&self) -> &str {
            match self {
                MachineToken::V1(..) => MachineToken::V1_PREFIX,
            }
        }
        ...
        pub fn decode(encoded: &str) -> Result<Self, ApiKeyError> {
            if let Some(rest) = encoded.strip_prefix(Self::V1_PREFIX) {
                ...
                return Ok(MachineToken::V1(credentials));
            }
...
mod test {
    ...
    use crate::credentials::{Credentials, MachineToken};
...
";
    ...
    fn v1_ci_api_key_encode_decode() {
        ...
        let api_key = MachineToken::decode(V1_API_KEY).expect("decode api key");
        let MachineToken::V1(credentials) = &api_key;
        assert_eq!(
